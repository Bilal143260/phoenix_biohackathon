...
from optimizer import optimize_media, OptimizationConfig, OptimizationResult
...
DEFAULTS = {
    ...
    # AI Optimizer
    "ai_opt_result": None,        # OptimizationResult from Bayesian opt
    "ai_opt_running": False,
}
...
# ─────────────────────────────────────────────
# PAGE TABS: Manual Simulator | AI Optimizer
# ─────────────────────────────────────────────
page_tab_simulator, page_tab_ai = st.tabs(["Manual Simulator", "AI Media Optimizer"])

with page_tab_simulator:
...
# ─────────────────────────────────────────────
# AI MEDIA OPTIMIZER TAB
# ─────────────────────────────────────────────
with page_tab_ai:
    st.markdown("""
    <div class="section-card">
        <div class="section-title">AI Media Optimizer</div>
        <div class="section-subtitle">
            Uses Bayesian Optimization (Optuna) to automatically search for the best media
            composition under a budget constraint. Upload a GEM model first, then configure
            your budget and let the AI find the optimal growth conditions.
        </div>
    </div>
    """, unsafe_allow_html=True)

    if st.session_state.cobra_model is None or st.session_state.upload_mode != "gem":
        st.markdown("""
        <div class="empty-state">
            <div class="empty-state-icon">🤖</div>
            <div class="empty-state-text">Upload a GEM model first</div>
            <div class="empty-state-hint">
                Go to the Manual Simulator tab and upload an SBML model to enable AI optimization.
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        medium_components = st.session_state.gem_medium
        model = st.session_state.cobra_model

        # ── Configuration ──
        st.markdown("""
        <div class="section-card" style="padding-bottom:1rem;">
            <div class="section-title" style="font-size:1.1rem;">Optimization Setup</div>
            <div class="section-subtitle">
                Define a budget constraint so the optimizer can't simply set everything to infinity.
                Then configure the search and hit run.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with cfg_col1:
            budget_mode = st.selectbox(
                "Budget type",
                options=["Total Flux Limit", "Cost-Weighted Budget"],
                help=(
                    "**Total Flux Limit**: Sum of all uptake bounds cannot exceed the budget. "
                    "**Cost-Weighted Budget**: Assign a cost per metabolite; total cost cannot exceed budget."
                ),
            )
            is_cost_mode = budget_mode == "Cost-Weighted Budget"

            current_total = sum(c.current_bound for c in medium_components)
            budget_max = max(round(current_total * 2.0, 0), 10000.0)
            budget = st.number_input(
                "Budget" + (" ($)" if is_cost_mode else " (mmol/gDW/h total)"),
                min_value=1.0,
                max_value=budget_max,
                value=round(current_total * 1.0, 0),
                step=10.0,
                help=f"Current default total: {current_total:.0f}",
            )
        ...
        with cfg_col2:
            n_trials = st.slider(
                ...
                value=60,
                step=10,
                help="More trials = better results but longer runtime. 50-100 is usually good.",
            )

            max_bound = st.number_input(
                "Max bound per component (mmol/gDW/h)",
                min_value=1.0,
                max_value=2000.0,
                value=1000.0,
                step=50.0,
                help="Upper limit for any single exchange reaction bound.",
            )

        # ── Cost editor (only in cost mode) ──
        costs = {}
        if is_cost_mode:
            st.markdown("""
            <div class="section-card" style="padding-bottom:1rem;">
                <div class="section-title" style="font-size:1.1rem;">Metabolite Costs</div>
                <div class="section-subtitle">
                    Assign a cost per unit for each medium component. Cheap nutrients
                    (glucose, salts) should have low values; expensive ones (amino acids,
                    vitamins) should be higher.
                </div>
            </div>
            """, unsafe_allow_html=True)

            cost_cols = st.columns(3, gap="small")
            for idx, comp in enumerate(medium_components):
                col = cost_cols[idx % 3]
                with col:
                    display_name = comp.name if comp.name != comp.reaction_id else comp.reaction_id
                    c = st.number_input(
                        ...
                        value=1.0,
                        step=0.5,
                        key=f"cost_{comp.reaction_id}",
                    )
                    costs[comp.reaction_id] = c
        ...
        with run_col1:
            run_clicked = st.button(
                ...
                type="primary",
                use_container_width=True,
                disabled=st.session_state.ai_opt_running,
            )

        with run_col2:
            if st.session_state.ai_opt_result is not None:
                if st.button("🔄 Reset & Re-configure", type="secondary", use_container_width=True):
                    st.session_state.ai_opt_result = None
                    st.rerun()

        if run_clicked:
            config = OptimizationConfig(
                budget=budget,
                budget_mode="cost" if is_cost_mode else "flux",
                costs=costs,
                n_trials=n_trials,
                max_bound=max_bound,
            )

            st.session_state.ai_opt_running = True
            progress_bar = st.progress(0, text="Starting Bayesian optimization...")
            best_display = st.empty()

            def on_trial(trial_num, total, best_so_far):
                frac = trial_num / total
                progress_bar.progress(frac, text=f"Trial {trial_num}/{total}")
                best_display.markdown(
                    f"<div style='font-size:0.85rem; color:#53b2a6;'>"
                    f"Best growth rate so far: <strong>{best_so_far:.4f} h⁻¹</strong></div>",
                    unsafe_allow_html=True,
                )
            ...
            try:
                ...
                st.session_state.ai_opt_result = result
            except Exception as exc:
                st.error(f"Optimization failed: {exc}")
            finally:
                st.session_state.ai_opt_running = False
                progress_bar.empty()
                best_display.empty()
                st.rerun()
        ...
        # ── Results ──
        if st.session_state.ai_opt_result is not None:
            opt_res = st.session_state.ai_opt_result

            st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
            st.markdown("""
            <div class="section-card" style="padding-bottom:1.5rem;">
                <div class="section-title">AI Optimization Results</div>
                <div class="section-subtitle">
                    The best media composition found by Bayesian optimization under your budget constraint.
                </div>
            </div>
            """, unsafe_allow_html=True)

            # ── KPIs ──
            feasible_count = sum(1 for t in opt_res.trials if t.feasible)
            k1, k2, k3, k4 = st.columns(4, gap="medium")
            with k1:
                st.markdown(f"""
                <div class="kpi-card">
                    <div class="kpi-label">Best Growth Rate</div>
                    <div class="kpi-value" style="color:#16a34a;">{opt_res.best_growth_rate:.4f}</div>
                    <div class="kpi-unit">h⁻¹</div>
                </div>
                """, unsafe_allow_html=True)
            with k2:
                budget_label = "$" if opt_res.config.budget_mode == "cost" else "flux"
                st.markdown(f"""
                <div class="kpi-card">
                    <div class="kpi-label">Budget Used</div>
                    <div class="kpi-value">{opt_res.best_cost:.1f}</div>
                    <div class="kpi-unit">of {opt_res.config.budget:.0f} {budget_label}</div>
                </div>
                """, unsafe_allow_html=True)
            with k3:
                st.markdown(f"""
                <div class="kpi-card">
                    <div class="kpi-label">Trials Run</div>
                    <div class="kpi-value">{len(opt_res.trials)}</div>
                    <div class="kpi-unit">{feasible_count} feasible</div>
                </div>
                """, unsafe_allow_html=True)
            with k4:
                ...
                try:
                    default_fba = run_fba(model, default_bounds)
                    default_gr = default_fba.growth_rate if default_fba.status == "optimal" else 0.0
                except Exception:
                    default_gr = 0.0
                if default_gr > 0:
                    improvement = ((opt_res.best_growth_rate - default_gr) / default_gr) * 100
                    imp_str = f"{improvement:+.1f}%"
                    imp_color = "#16a34a" if improvement > 0 else "#ef4444"
                else:
                    imp_str = "N/A"
                    imp_color = "#9ca3af"
                st.markdown(f"""
                <div class="kpi-card">
                    <div class="kpi-label">vs Default Medium</div>
                    <div class="kpi-value" style="color:{imp_color};">{imp_str}</div>
                    <div class="kpi-unit">growth change</div>
                </div>
                """, unsafe_allow_html=True)

            st.markdown("")

            # ── Convergence Plot + Best Bounds ──
            conv_col, bounds_col = st.columns([3, 2], gap="large")

            with conv_col:
                st.markdown("""
                <div class="section-title" style="font-size:1.1rem;">Optimization Convergence</div>
                <div class="section-subtitle" style="margin-bottom:1rem;">
                    Growth rate over trials. The algorithm learns which regions of the search space are promising.
                </div>
                """, unsafe_allow_html=True)
                ...
                running_best = []
                best_so_far = 0.0
                for t in opt_res.trials:
                    trial_nums.append(t.number + 1)
                    trial_growth.append(t.growth_rate if t.feasible else 0.0)
                    if t.feasible and t.growth_rate > best_so_far:
                        best_so_far = t.growth_rate
                    running_best.append(best_so_far)

                fig = go.Figure()
                fig.add_trace(go.Scatter(
                    x=trial_nums, y=trial_growth,
                    mode="markers",
                    name="Trial growth",
                    marker=dict(
                        size=6,
                        color=["rgba(83,178,166,0.5)" if g > 0 else "rgba(239,68,68,0.3)" for g in trial_growth],
                    ),
                ))
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines",
                    name="Best so far",
                    line=dict(color="#53b2a6", width=2.5),
                ))
                fig.update_layout(
                    height=300,
                    margin=dict(l=0, r=20, t=10, b=10),
                    xaxis=dict(
                        title=dict(text="Trial", font=dict(size=11, color="#9ca3af")),
                        showgrid=True, gridcolor="rgba(0,0,0,0.04)",
                        tickfont=dict(size=10, color="#9ca3af"),
                    ),
                    yaxis=dict(
                        title=dict(text="Growth Rate (h⁻¹)", font=dict(size=11, color="#9ca3af")),
                        showgrid=True, gridcolor="rgba(0,0,0,0.04)",
                        tickfont=dict(size=10, color="#9ca3af"),
                    ),
                    plot_bgcolor="rgba(0,0,0,0)",
            ...
            with bounds_col:
                st.markdown("""
                <div class="section-title" style="font-size:1.1rem;">Optimal Media Bounds</div>
                <div class="section-subtitle" style="margin-bottom:1rem;">
                    AI-recommended uptake bounds vs the original defaults.
                </div>
                """, unsafe_allow_html=True)

                for comp in medium_components:
                    opt_bound = opt_res.best_bounds.get(comp.reaction_id, 0.0)
                    orig_bound = comp.current_bound
                    display_name = comp.name if comp.name != comp.reaction_id else comp.reaction_id

                    if opt_bound > orig_bound * 1.05:
                        arrow = "▲"
                        change_color = "#16a34a"
                    elif opt_bound < orig_bound * 0.95:
                        arrow = "▼"
                        change_color = "#3b82f6"
                    else:
                        arrow = "–"
                    ...
                    <div class="range-card" style="margin-bottom:0.4rem; text-align:left; padding:0.6rem 1rem;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <span style="font-weight:600; color:#1a1a2e; font-size:0.82rem;">{display_name}</span>
                            <span style="font-size:0.82rem; color:{change_color}; font-weight:600;">{arrow} {opt_bound:.1f}</span>
                        </div>
                        <div style="font-size:0.7rem; color:#9ca3af; margin-top:0.15rem;">
                            Default: {orig_bound:.1f} · {comp.reaction_id}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            st.markdown("")
            st.markdown("""
            <div class="section-title" style="font-size:1.1rem;">Default vs AI-Optimized Bounds</div>
            <div class="section-subtitle" style="margin-bottom:1rem;">
                Side-by-side comparison of original and optimized uptake bounds.
            </div>
            """, unsafe_allow_html=True)
            ...
            for comp in medium_components:
                display_name = comp.name if comp.name != comp.reaction_id else comp.reaction_id
                comp_names.append(display_name)
                default_vals.append(comp.current_bound)
                optimized_vals.append(opt_res.best_bounds.get(comp.reaction_id, 0.0))

            fig2 = go.Figure()
            fig2.add_trace(go.Bar(
                name="Default", x=comp_names, y=default_vals,
                marker_color="rgba(156,163,175,0.5)",
            ))
            fig2.add_trace(go.Bar(
                name="AI-Optimized", x=comp_names, y=optimized_vals,
                marker_color="rgba(83,178,166,0.8)",
            ))
            fig2.update_layout(
                barmode="group",
                height=300,
                margin=dict(l=0, r=20, t=10, b=10),
                xaxis=dict(tickfont=dict(size=10, color="#374151"), tickangle=-45),
                yaxis=dict(
                    title=dict(text="Bound (mmol/gDW/h)", font=dict(size=11, color="#9ca3af")),
                    showgrid=True, gridcolor="rgba(0,0,0,0.04)",
                    tickfont=dict(size=10, color="#9ca3af"),
                ),
                plot_bgcolor="rgba(0,0,0,0)",
            ...
            ai_structured = {
                "ai_media_optimizer": {
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "config": {
                        "budget": opt_res.config.budget,
                        "budget_mode": opt_res.config.budget_mode,
                        "n_trials": opt_res.config.n_trials,
                        "max_bound": opt_res.config.max_bound,
                    },
                    "result": {
                        "best_growth_rate": round(opt_res.best_growth_rate, 6),
                        "best_cost": round(opt_res.best_cost, 2),
                        "best_bounds": {k: round(v, 3) for k, v in opt_res.best_bounds.items()},
                        "total_trials": len(opt_res.trials),
                        "feasible_trials": sum(1 for t in opt_res.trials if t.feasible),
                    },
                }
            }
            ai_json = json.dumps(ai_structured, indent=2)
            ...
            for comp in medium_components:
                ai_csv_rows.append({
                    "Reaction_ID": comp.reaction_id,
                    "Name": comp.name,
                    "Default_Bound": comp.current_bound,
                    "AI_Optimized_Bound": round(opt_res.best_bounds.get(comp.reaction_id, 0.0), 3),
                    "Unit": "mmol/gDW/h",
                })
            ai_csv = pd.DataFrame(ai_csv_rows).to_csv(index=False)

            dl1, dl2, dl3 = st.columns([1, 1, 3], gap="medium")
            with dl1:
                st.download_button(
                    "📥 Download JSON",
                    data=ai_json,
                    file_name="ai_optimizer_results.json",
                    mime="application/json",
                    use_container_width=True,
                )
            with dl2:
...
st.markdown(
    ...
    "GEM Media Optimizer · Hackathon MVP · Built with Streamlit, COBRA & Optuna"
    "</div>",
    unsafe_allow_html=True,
)
